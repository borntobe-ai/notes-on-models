...
[Code Source](https://github.com/karpathy/llm.c)

I want to go over the essense of LLMs first, so a reader can continue reading with an idea how the potential product could be.

A llm model is trained in the following steps in general:

1. Define the architecture of the model, as in layers of calculation
2. Label data as input and output pairs
3. Pass the input data to the model then the model estimate an output
4. Compare the calculated output with the labeled desired output, result as a loss
5. The loss is send back into the mode backwards of the layers
6. The model then adjusts the weights at each layer
7. Send in massive inputs and outputs for continus training
8. Model ready
